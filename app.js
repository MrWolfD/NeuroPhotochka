...
async function copyCurrentPrompt() {
  ...
  // ✅ Обновляем счетчик ТОЛЬКО если бэкенд реально записал копирование (первый раз для пользователя)
  try {
    const res = await callEdge(PROMPT_COPY_URL, { prompt_id: prompt.id });

    // ожидаем { ok:true, incremented:true/false, copies_count?:number }
    const incremented = Boolean(res?.incremented ?? res?.created ?? res?.inserted ?? false);
    const newCount = (typeof res?.copies_count === 'number') ? res.copies_count : null;

    if (incremented) {
      if (typeof newCount === 'number') {
        prompt.copies = newCount;
      } else {
        prompt.copies = (Number(prompt.copies) || 0) + 1;
      }

      // обновляем счетчик в модалке (она открыта, раз мы тут)
      const el = document.getElementById('promptModalCopies');
      if (el) el.textContent = String(prompt.copies || 0);

      // ✅ точечное обновление карточки + возможное перемещение (без мерцания)
      onPromptMetricsChanged(prompt.id);
    }
  } catch (e) {
    console.warn("prompt_copy failed:", e);
    // если логирование на бэке упало — UI не трогаем, чтобы не расходиться с БД
  }
}
...
async function copyPromptDirectly(promptId) {
  ...
  try {
    ...
    if (incremented) {
      ...
      // если открыт модал — синхроним и его тоже
      if (dom.promptModalOverlay?.classList.contains('show')) {
        const list = state.filteredPrompts.length ? state.filteredPrompts : state.prompts;
        const cur = list[modal.currentIndex];
        if (cur && cur.id === prompt.id) {
          const el = document.getElementById('promptModalCopies');
          if (el) el.textContent = String(prompt.copies || 0);
        }
      }
